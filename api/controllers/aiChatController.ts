...
import { generateResponse, generateFollowUpQuestions } from "../services/gemini.service";
 ...
 */
export async function postChat(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    // Fetch distinct cities from database using aggregation (stable API v1 compatible)
...
User query: ${message}`;
    ...
    if (shouldStream) {
      ...
      try {
        assistantReply = await generateResponse(
          systemInstruction,
          historyParams,
          decoratedMessage,
          (chunkText) => {
        ...
        const suggestedFollowUps = await generateFollowUpQuestions(updatedHistory, assistantReply);
 ...
 */
export async function deleteChatHistory(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    const { conversationId } = req.params;
    const db = await connectToDatabase();
